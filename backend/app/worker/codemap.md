...
- **退避重启**：error 且 `retryable=True` → 延迟 `min(10·2^(n-1), 300)`s（首次
  delay=0）置回 starting；**拒启类**（模型缺失/显存不足/Popen 失败）置
  `retryable=False` 不自动重启（防绕过显存双保险）；重启前 pre-kill 存活旧进程。
...
- **主循环**（`main.py:worker_lifespan`）：注册（失败重试耗尽 → 启动失败退出）→
  restore running 实例 → 3 个后台任务并行（异常不退出、shutdown cancel）：
  `status_loop`（采集上报，`WORKER_STATUS_INTERVAL`，承担存活语义）/
  `report_loop`（实例对账，`WORKER_REPORT_INTERVAL`，快照为空跳过）/ `sync_loop`
  （状态机权威判定，5s）
- **实例生命周期**（`lifecycle.py:start` 受理 7 步，契约 §2.2）：
  ① 模型路径解析（`resolve_model_path`，不存在 → ERROR + retryable=False；根外绝对
     路径容器挂载不可见 → 拒启）
  ...
  （≈10s）或总时长超 30s → error「启动超时」；running 失活连续 ≥2 → error「进程失活/
  健康检查失败」；error → 退避重启（拉起前入口无条件 stop_container 清理同名残留）。
...
- **停止/恢复/权重**（`lifecycle.py`）：
  - `stop`：幂等（不存在 → accepted）；`stop_container`（`docker stop -t 3` → 超时
    ...
    清记录/端口/meta → report 消失 → server 收敛 stopped。
