...
2. **声明式显存记账**（K8s Node.Allocated 模式）：谁被分配了什么，由**管理端 DB 说了算**（`allocated = Σ 实例声明`），不信 worker 自报。`available = total - allocated - reserved`，心跳的 `used` 只做展示。
3. **实际状态由最接近真相的一方写入**：worker 直接回写实例实际状态，server 只做对账与节点失联时的主动干预。
...
| GPU 采集字段标准化 | `{index, total, used, util, temperature, power}`，单位统一 Bytes，展示层换算 GiB | worker 上报结构对齐，`index` 必须保留（分配依赖） |
...
- **需求**：`vram_claim = weight×1.2 + 2GiB`（weight 由 worker 统计 `.safetensors/.bin/.pt/.pth` 求和）；用户填 GMU（默认 0.9）
...
1. **契约与鉴权**：注册 token、心跳/状态分离、`compute_state()`、`request_to_worker()` 转发
2. **worker 生命周期**：健康检查、状态机、对账 sync_loop、退避重启、显存启动前校验
3. **server 显存分配**：GPU 记账表、需求估算、first-fit 选卡、参数注入
