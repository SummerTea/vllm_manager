...
vLLM 推理服务集群管理平台：一个 server 管理端（Web + REST，:8000）通过各 GPU 机器上的 worker 管控节点（:8100）拉起/停止/监控 **vLLM 推理实例**，并按**显存**做分配决策。server 持有权威账本（PostgreSQL），worker 状态全内存经 report 对账收敛，实例生命周期（启动/健康检查/退避重启/失联联动）闭环于两端的契约状态机。**已在本机 colima/CPU 与真实 GPU 靶场（A800 2×80GB，见 `docs/a800-worker-deploy.md`/`docs/a800-environment.md`）全链路验证**。
...
| backend worker | `backend/app/work_worker.py`（`uvicorn app.worker.main:create_app --factory --host 0.0.0.0 --port 8100`） | 8100 | GPU 机器管控节点；不连 PG，状态全内存，3 后台循环（status/report/sync）；依赖 docker 拉起 vLLM 容器（GPU `--gpus device=`/`--network host`、CPU `-p` 映射；A800 GPU 真机与 CPU 本地均验证） |
...
- **契约文档**：`docs/architecture.md`（蓝图层）、`docs/worker-contract.md`（worker 字段级规格）、`docs/gpustack-borrowings.md`（借鉴调研）、`docs/local-testing.md`（本机 CPU 集成测试 runbook + §8 A800 GPU 结论）、`docs/a800-worker-deploy.md`（A800 集成 runbook）、`docs/a800-environment.md`（A800 主机环境台账）
...
| `backend/app/utils/` | 纯工具层：request_to_worker（server→worker 统一转发，Bearer+超时+异常映射；当前唯一实现，concurrency/formatters 已删） | [View Map](backend/app/utils/codemap.md) |
| `backend/app/main/` | 应用装配层：app 单例（Session/load_dotenv）、lifespan 初始化顺序、routers/web_routers 聚合、SafeStaticFiles SPA、全局异常 handler（含 worker 错误码透传） | [View Map](backend/app/main/codemap.md) |
| `backend/app/server/` | server 业务域总览：node←instance←allocator 单向依赖、对账闭环、路由/后台任务挂载 | [View Map](backend/app/server/codemap.md) |
| `backend/app/server/node/` | GPU 节点域：Node.compute_state 派生（pending/ready/offline/unreachable）、幂等注册、/healthz 主动探测、Bearer 鉴权；删除=机器退役（存活守卫 READY/UNREACHABLE→409 + 级联删 stopped 实例） | [View Map](backend/app/server/node/codemap.md) |
| `backend/app/server/allocator/` | 显存分配决策（纯函数）：first_fit 单卡/多卡 TP、记账口径 available=total−Σallocated−reserved（**不消费 memory_used 外部占用**）、仅显式 accelerator=cpu 走 CPU 分支（其余 fail-closed） | [View Map](backend/app/server/allocator/codemap.md) |
| `backend/app/server/instance/` | vLLM 实例域：InstanceLifecycleMixin+状态机纯函数、VllmInstance 表、/instances 管理端点 + /report 对账端点（stop 悬挂限次重下发）、失联联动（reconcile_node_loss/reconcile_lost_nodes，prober 回调驱动） | [View Map](backend/app/server/instance/codemap.md) |
| `backend/app/server/instance/service/` | 实例服务层（目录化）：creation.py 8 步 create 编排（候选→权重广播→**advisory lock 串行 + 指定 node_id 防 fallthrough**→allocator→建记录→转发 start）、VllmInstanceService 生命周期方法（含 delete_stopped_by_node 级联） | [View Map](backend/app/server/instance/service/codemap.md) |
| `backend/app/worker/` | worker 域：GPU 机器 vLLM 实例生命周期管控节点（内存态状态机+退避重启+restore 仅 running+report 对账闭环）；GPU/CPU 双模板（内引号多卡 `--gpus`、子目录模型名、CPU 分支不注入 OMP） | [View Map](backend/app/worker/codemap.md) |
